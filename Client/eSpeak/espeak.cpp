...
#ifdef _WIN32
HINSTANCE hDLL = LoadLibrary("espeak_sapi");               // Handle to DLL
#endif
...
#ifdef _WIN32
	typedef const espeak_VOICE ** (CALLBACK* ESPEAK_LISTVOICES)(espeak_VOICE *);
	ESPEAK_LISTVOICES espeak_ListVoices;    // Function pointer
	if (hDLL != NULL) {
		espeak_ListVoices = (ESPEAK_LISTVOICES)GetProcAddress(hDLL, "espeak_ListVoices");
	    if (!espeak_ListVoices)
			// handle the error
			FreeLibrary(hDLL);
	}
#endif
...
{
#ifdef _WIN32
	typedef void (CALLBACK* ESPEAK_COMPILEDICTIONARY)(const char *, FILE *, int);
	ESPEAK_COMPILEDICTIONARY espeak_CompileDictionary;
	typedef const char * (CALLBACK* ESPEAK_INFO)(void*);
	ESPEAK_INFO espeak_Info;
	typedef int (CALLBACK* ESPEAK_INITIALIZE)(espeak_AUDIO_OUTPUT, int, const char *, int);
	ESPEAK_INITIALIZE espeak_Initialize;
	typedef espeak_ERROR (CALLBACK* ESPEAK_SETPARAMETER)(espeak_PARAMETER, int, int);
	ESPEAK_SETPARAMETER espeak_SetParameter;
	typedef void (CALLBACK* ESPEAK_SETPHONEMETRACE)(int, FILE *);
	ESPEAK_SETPHONEMETRACE espeak_SetPhonemeTrace;
	typedef espeak_ERROR (CALLBACK* ESPEAK_SETPUNCTUATIONLIST)(const wchar_t *);
	ESPEAK_SETPUNCTUATIONLIST espeak_SetPunctuationList;
	typedef espeak_ERROR (CALLBACK* ESPEAK_SETVOICEBYNAME)(const char *);
	ESPEAK_SETVOICEBYNAME espeak_SetVoiceByName;
	typedef espeak_ERROR (CALLBACK* ESPEAK_SETVOICEBYPROPERTIES)(espeak_VOICE *);
	ESPEAK_SETVOICEBYPROPERTIES espeak_SetVoiceByProperties;
	typedef espeak_ERROR (CALLBACK* ESPEAK_SYNCHRONIZE)();
	ESPEAK_SYNCHRONIZE espeak_Synchronize;
	typedef espeak_ERROR (CALLBACK* ESPEAK_SYNTH)(const void *,  size_t, unsigned int, espeak_POSITION_TYPE, unsigned int, unsigned int, unsigned int *, void *);
	ESPEAK_SYNTH espeak_Synth;
	typedef void (CALLBACK* ESPEAK_SETSYNTHCALLBACK)(t_espeak_callback *);
	ESPEAK_SETSYNTHCALLBACK espeak_SetSynthCallback;
	if (hDLL != NULL) {
		espeak_CompileDictionary = (ESPEAK_COMPILEDICTIONARY)GetProcAddress(hDLL, "espeak_CompileDictionary");
		espeak_Info = (ESPEAK_INFO)GetProcAddress(hDLL, "espeak_Info");
		espeak_Initialize = (ESPEAK_INITIALIZE)GetProcAddress(hDLL, "espeak_Initialize");
		espeak_SetParameter = (ESPEAK_SETPARAMETER)GetProcAddress(hDLL, "espeak_SetParameter");
		espeak_SetPhonemeTrace = (ESPEAK_SETPHONEMETRACE)GetProcAddress(hDLL, "espeak_SetPhonemeTrace");
		espeak_SetPunctuationList = (ESPEAK_SETPUNCTUATIONLIST)GetProcAddress(hDLL, "espeak_SetPunctuationList");
		espeak_SetVoiceByName = (ESPEAK_SETVOICEBYNAME)GetProcAddress(hDLL, "espeak_SetVoiceByName");
		espeak_SetVoiceByProperties = (ESPEAK_SETVOICEBYPROPERTIES)GetProcAddress(hDLL, "espeak_SetVoiceByProperties");
		espeak_Synchronize = (ESPEAK_SYNCHRONIZE)GetProcAddress(hDLL, "espeak_Synchronize");
		espeak_Synth = (ESPEAK_SYNTH)GetProcAddress(hDLL, "espeak_Synth");
		espeak_SetSynthCallback = (ESPEAK_SETSYNTHCALLBACK)GetProcAddress(hDLL, "espeak_SetSynthCallback");

	    if (!espeak_CompileDictionary || !espeak_Info || !espeak_Initialize || !espeak_SetParameter || !espeak_SetPhonemeTrace || !espeak_SetPunctuationList || !espeak_SetVoiceByName || !espeak_SetVoiceByProperties || !espeak_Synchronize || !espeak_Synth || !espeak_SetSynthCallback)
			FreeLibrary(hDLL);
	}
#endif
