...
void *speak_thread(void *ptr)
{
    string path;
#ifdef _WIN32
    path = string("--path=" + string(_getcwd(NULL, 0)));
#elif defined(__APPLE__)
    path = string("--path=" + string(getwd(NULL)) + "/PUSPA.app/Contents/Resources");
#else
    path = "--path=/usr/local/share/PUSPA";
#endif

    const char *arguments[] = {"", path.c_str(), "-vid", (const char*)ptr};
    speak(sizeof(arguments) / sizeof(arguments[0]), const_cast<char **> (arguments));
    free(ptr);
}
