...
//  int len;
  ...
  for(r=recog->process_list;r;r=r->next) {
    ...
    for(n = 0; n < r->result.sentnum; n++) { /* for all sentences */
      ...
      char *words = NULL;
      for(i=0;i<seqnum;i++) {
          char *woutput = winfo->woutput[seq[i]];
          printf(" %s", woutput);
          if (strcmp(woutput, "<sil>") && strcmp(woutput, "</sil>")) {
              if (!words) {
                  words = (char *) malloc(strlen(woutput)+1);
                  strcpy(words, woutput);
              } else {
                  words = (char *) realloc(words, strlen(words)+strlen(woutput)+2);
                  strcat(words, " ");
                  strcat(words, woutput);
              }
          }
      }
      handleText(words, (Agent *)dummy);
      free(words);
      words = NULL;
      printf("\n");
      /* LM entry sequence */
...
hear(void *ptr)
{
    ...
    char *jconf_path = "/usr/local/share/PUSPA/julius-data/julian.jconf";
    ...
    callback_add(recog, CALLBACK_RESULT, output_result, ptr);
